...
from modules.coord_req import CoordReq


class NodeChoice:
    """Make choices on existing nodes"""
    ...
    def __init__(self, parent, node):
        ...
        self.rbtn1 = tk.Radiobutton(
            self.root, text="Link", variable=self.choice, value=0
        )

        self.rbtn1.grid(row=1, column=0)

        self.rbtn2 = tk.Radiobutton(
            self.root, text="Rename", variable=self.choice, value=1
        )
    ...
    def on_button_press(self):
        ...
        if self.choice == 1:
            CoordReq(self.parent, self.node)
