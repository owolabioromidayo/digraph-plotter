...
class Node:
    ...
    def create_link(self, graph):
        """Different ways of linking nodes without passing centers and obstructing views of info contained within"""
        ...
        if abs(self.prevnode.y -self.y) <= 100:
            ...
            if self.prevnode.x < self.x:
                graph.create_line(self.x-50, self.y, self.prevnode.x+50, self.prevnode.y, arrow=tk.FIRST)
            ...
            graph.create_line(self.x, self.y+20, self.prevnode.x, self.prevnode.y-20, arrow=tk.FIRST)
